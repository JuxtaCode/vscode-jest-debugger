...
function getCodeLensForOption(range: Range, codeLensOption: CodeLensOption, fullTestName: string): CodeLens {
  ...
  const runLabel = config.runLabel;
  const debugLabel = config.debugLabel;

  const titleMap: Record<CodeLensOption, string> = {
    run: runLabel ? runLabel : '🚀Run',
    debug: debugLabel ? debugLabel : '🕷️Debug',
    watch: 'Watch',
  };
...
export class JestDebuggerCodeLensProvider implements CodeLensProvider {
